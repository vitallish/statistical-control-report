...
The false positive rate is defined as either finding a trend based (6 points in a row on one side of the centerline) or an extreme value happening outside of the control limits that occured during a section where the `true_val` was not different than baseline. A low rate is desirable.
...
```{r}
tree_mod <- rpart(as.factor(true_positive) ~ data_sparcity  + observations_per_day + 
                    data_density + process_start + 
           process_change + stat_start + stat_change + 

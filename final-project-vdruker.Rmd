...
generateData <- function(data_sparcity = 0.1,
  ...
  d <- d %>% 
    ...
    mutate(dt_week = floor_date(dt, unit = "week")) %>% 
...
createQCC <- function(d, cutoff = NULL){
  ...
  if(is.null(cutoff)){
    out <- qcc.formula(data_samp ~ dt_week, data = d, type = type)
  }else{
    out <- qcc.formula(data_samp ~ dt_week, data = d, cutoff = cutoff, type = type)
  }
  
  out
}

evalQCC  <- function(d, qcc_obj, invalid_runs = c("Violating Run", "Beyond Limits")){
  true_stats <- d %>% 
    group_by(dt_week) %>% 
  ...
  calc_df <-  augment.qcc(qcc_obj, gr_func = as.Date) %>% 
    rename(dt_week = group) %>% 
    mutate(dt_week = dt_week + days(1)) %>% 
    left_join(true_stats, by = "dt_week") 
  ...
  arl <- ifelse(length(first_detect) == 1, as.numeric(first_detect - emp_process_start, unit = "days"), Inf)

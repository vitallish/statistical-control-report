...
```{r}
tree_mod <- train(as.factor(fitted) ~ data_sparcity *observations_per_day + process_start + 
           process_change + stat_start + stat_change + 
           stat_change_type + data_type + date_floor, data = all_data, method = "rpart")

rpart.plot::rpart.plot(tree_mod$finalModel, left = FALSE, xflip = TRUE)
```


```{r}
tree_mod_fp %<-% {train(as.factor(false_positive) ~ data_sparcity *observations_per_day + process_start + 
           process_change + stat_start + stat_change + 
           stat_change_type + data_type + date_floor, data = all_data, method = "rpart",
           na.action = na.omit)}

rpart.plot::rpart.plot(tree_mod_fp$finalModel)
```


```{r}
tree_mod_tp %<-% {train(as.factor(true_positive) ~ data_sparcity*observations_per_day + process_start + 
           process_change + stat_start + stat_change + 
           stat_change_type + data_type + date_floor, data = all_data, method = "rpart",
           na.action = na.omit)}

rpart.plot::rpart.plot(tree_mod_tp$finalModel)
```


```{r}
tree_mod <- train(arl ~ data_sparcity*observations_per_day + process_start + 
           process_change + stat_start + stat_change + 
           stat_change_type + data_type + date_floor, data = filter(all_data, !is.infinite(arl)), method = "rpart",
           na.action = na.omit)
